# SellerFlow Parcel Checker

Chrome extension that checks SellerFlowLive parcel scans against 7-11 賣貨便 (v1.4.0).

> The old "SellerFlow Live Comment" TikTok panel was removed in v1.4.0 — this
> extension is now parcel-checker only.

## Install In Chrome

1. Open Chrome → `chrome://extensions` → turn on `Developer mode`.
2. Click `Load unpacked` → select this folder: `chrome-extension`.
3. Follow "One-time setup (popup)" below (keep the SellerFlowLive, myship, and
   emap tabs open + logged in).

---

# Parcel checker — 賣貨便 full-store + restricted-phone

Checks each new `parcel_scans` row against 7-11's 賣貨便 using **your logged-in
賣貨便 session** —
...
- **`emap-711.js`** (on `emap.pcsc.com.tw`) — **Full store:**
  `POST /ecmap/byIDData.aspx` → field 3 `enable`→open / `disable`→full (any other value → unknown). `eshopGuid`
