...
- **Add button → confirmation popup** (centered modal, redesign-styled — NOT the yellow of the source screenshot): title (e.g. "Add TikTok — Multi Account"), body *"To add a … for LIVE multi-account, please message our admin on Telegram and we'll set it up for you."*, a Telegram chip showing **@sellerflowlive1995**, and two actions: **Cancel** (dismiss) and **OK** → opens `https://t.me/sellerflowlive1995` in a new tab. State: `ttAddOpen` / `fbAddOpen`; screens `ttchannels` / `fbchannels`; data from `TT_ACCOUNTS` / `FB_ACCOUNTS`.
