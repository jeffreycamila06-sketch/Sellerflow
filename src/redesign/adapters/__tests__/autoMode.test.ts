...
describe("planAutoOrder — match + claim decision (PURE)", () => {
  ...
  it("code with stock → order (1 piece), with nextStock + soldOut flag", () => {
...
describe("NO quantity syntax — every auto order is ONE piece", () => {
  const plenty = () => 50;
  it("'<code> <n>' is a plain comment (no order) — never a quantity", () => {
    for (const text of ["D 2", "d 2", "A 5", "BB 99", "D  2", " D 2 "]) {
      expect(planAutoOrder(text, CODES, plenty)).toEqual({ kind: "none" });
    }
  });
  it("other quantity-looking forms are plain comments too", () => {
    for (const text of ["D+2", "D x2", "Dx2", "D*2", "2 D", "D 0", "D 100", "D two"]) {
      expect(planAutoOrder(text, CODES, plenty)).toEqual({ kind: "none" });
    }
  });
  it("'A12' only ever matches a code literally named A12 — never 'A1' + 2", () => {
    const codes = [code({ code: "A1", productLocalId: 1 }), code({ code: "A12", productLocalId: 2 })];
    expect(planAutoOrder("A12", codes, plenty)).toMatchObject({ kind: "order", code: codes[1] });
    expect(planAutoOrder("A1 2", codes, plenty)).toEqual({ kind: "none" });
  });
  it("a code the seller literally named with a space still matches EXACTLY", () => {
    const codes = [code({ code: "A1 2", productLocalId: 2 })];
    expect(planAutoOrder("a1 2", codes, plenty)).toMatchObject({ kind: "order", code: codes[0] });
  });
  it("a matched code always takes exactly ONE piece of stock; there is no 'not enough stock' outcome", () => {
    expect(planAutoOrder("D", CODES, () => 1)).toEqual({ kind: "order", code: CODES[1], nextStock: 0, soldOut: true });
    expect(planAutoOrder("D", CODES, () => 0)).toEqual({ kind: "soldout", code: CODES[1] });
  });
});
