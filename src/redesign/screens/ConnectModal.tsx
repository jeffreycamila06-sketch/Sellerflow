...
export default function ConnectModal({ profile, initialTab = "TikTok", onClose, onConnect }: {
  ...
  const connect = async () => {
    ...
    setBusy(false);
    if (!r.ok) { setErr(r.error || "Connection failed."); return; }
    onClose();
  };
    ...
    <div onClick={(e) => e.target === e.currentTarget && !busy && onClose()} style={{ position: "absolute", inset: 0, zIndex: 1000, background: "rgba(8,6,24,.5)", backdropFilter: "blur(2px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 22 }}>
      <div style={{ width: "100%", maxWidth: 360, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 18, boxShadow: "0 24px 60px rgba(0,0,0,.4)", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "15px 16px 12px", borderBottom: "1px solid var(--border)" }}>
          <span style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 16, color: "var(--text)" }}>Connect live account</span>
        ...
        <div style={{ padding: 16 }}>
          {!canConnect && <div style={{ fontSize: 12, fontWeight: 600, color: "var(--warn)", background: "rgba(217,119,6,.12)", border: "1px solid var(--warn)", borderRadius: 10, padding: "9px 11px", marginBottom: 12 }}>⚠ Your plan's account limit is reached. Upgrade to add more.</div>}
          ...
          {canUseExisting ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
              <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Choose a registered {tab} account:</div>
              {registered.map((a) => {
                const on = a === current;
                return (
              ...
              {canAdd && tab === "TikTok" && <div><label style={lbl}>…or add a new TikTok username</label><input value={ttu} onChange={(e) => { setSelTT(""); setTtu(e.target.value); }} placeholder="e.g. duonglily_0708" style={input} /></div>}
              {canAdd && tab === "Facebook" && <div><label style={lbl}>…or add a new Facebook live video ID</label><input value={fbId} onChange={(e) => { setSelFB(""); setFbId(e.target.value); }} style={input} /></div>}
            </div>
          ) : tab === "TikTok" ? (
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11.5, color: "var(--warn)", background: "rgba(217,119,6,.1)", border: "1px solid var(--warn)", borderRadius: 9, padding: "8px 10px", marginBottom: 9 }}>⚠ Use the exact TikTok username that is going live.</div>
              <label style={lbl}>TikTok username</label>
              <input value={ttValue} onChange={(e) => setTtu(e.target.value)} placeholder="e.g. duonglily_0708" disabled={!canAdd} style={input} />
            </div>
          ) : (
            <div style={{ marginBottom: 12, display: "flex", flexDirection: "column", gap: 9 }}>
              <div><label style={lbl}>Facebook live video ID</label><input value={fbValue} onChange={(e) => setFbId(e.target.value)} disabled={!canAdd} style={input} /></div>
              <div><label style={lbl}>Access token</label><input value={fbTok} onChange={(e) => setFbTok(e.target.value)} type="password" disabled={!canConnect} style={input} /></div>
            </div>
          )}

          {err && <div style={{ fontSize: 12, fontWeight: 600, color: "var(--danger)", marginBottom: 10 }}>{err}</div>}
          <button onClick={() => void connect()} disabled={busy || !canConnect || !current.trim()} style={{ width: "100%", padding: "12px 0", border: "none", borderRadius: 12, background: "var(--accent)", color: "var(--accent-text)", fontFamily: "var(--font-ui)", fontSize: 13.5, fontWeight: 700, cursor: busy || !canConnect ? "default" : "pointer", opacity: busy || !canConnect || !current.trim() ? 0.6 : 1, boxShadow: "0 4px 14px var(--accent-soft)" }}>{busy ? "Connecting…" : `Connect ${tab}`}</button>
          <div style={{ fontSize: 10.5, color: "var(--text-muted)", textAlign: "center", marginTop: 9, lineHeight: 1.45 }}>Live connection runs on the SellerFlow server — only active in the published app, not this preview.</div>
        </div>
      </div>
    </div>
  );
