...
export default function DeleteAccount({ onBack, email = "", onConfirm }: {
  ...
  const submit = async () => {
    ...
    if (!r.ok) { setErr(r.error || "Delete failed. Please try again."); setBusy(false); }
    ...
    <div>
      ...
      <div style={{ padding: "18px 16px 24px" }}>
        <div style={{ background: "var(--danger-soft, rgba(225,29,72,.08))", border: "1px solid var(--danger)", borderRadius: 16, padding: 18, textAlign: "center" }}>
          ...
          <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 17, color: "var(--text)" }}>Delete this seller account</div>
          <div style={{ fontSize: 13, color: "var(--text-dim)", marginTop: 6, lineHeight: 1.55 }}>This will delete the seller account for <b style={{ color: "var(--text)" }}>{email || "your account"}</b>, sign out this browser, and remove support messages for this email where possible.</div>
        </div>

        <div style={{ ...card, marginTop: 14 }}>
          <div style={{ fontSize: 12.5, color: "var(--text-dim)", lineHeight: 1.55 }}>Order, buyer, payment, audit, or legal records may be kept if needed for business history, security, or law.</div>
        </div>

        <label style={{ fontSize: 12, fontWeight: 600, color: "var(--text-dim)", margin: "16px 2px 6px", display: "block" }}>Type your email to confirm</label>
        ...
        {err && <div style={{ fontSize: 12.5, fontWeight: 600, color: "var(--danger)", marginTop: 10 }}>Warning: {err}</div>}
        <button onClick={() => void submit()} disabled={!canDelete || busy} style={{ width: "100%", marginTop: 14, padding: "14px 0", border: "none", borderRadius: 13, background: "var(--danger)", color: "#fff", fontFamily: "var(--font-ui)", fontSize: 14, fontWeight: 700, cursor: canDelete && !busy ? "pointer" : "not-allowed", opacity: canDelete && !busy ? 1 : 0.5 }}>{busy ? "Deleting…" : "Delete my account"}</button>
