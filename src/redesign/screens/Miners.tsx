    ...
    <div>
      <div style={headerBar}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {onExport
            ? <button onClick={onExport} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 700, background: "rgba(255,255,255,.16)", border: "none", color: "var(--on-header)", padding: "6px 11px", borderRadius: 9, cursor: "pointer", fontFamily: "var(--font-ui)" }}>⬇ Export</button>
      ...
      <div style={{ padding: "14px 14px 22px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
          <div style={statCard}>
            ...
            {!live && <div style={{ fontSize: 11, fontWeight: 700, color: "var(--ok)", marginTop: 3 }}>▲ 12% this month</div>}
          </div>
          <div style={statCard}>
            <div style={statLbl}>Total orders</div>
            <div style={bigNum}>{live && stats ? fmt(stats.orders) : "3,947"}</div>
            {!live && <div style={{ fontSize: 11, fontWeight: 700, color: "var(--ok)", marginTop: 3 }}>▲ 8% this month</div>}
          </div>
          <div style={statCard}>
            <div style={statLbl}>Total spent</div>
            <div style={{ ...bigNum, fontSize: 22 }}>{cur}{live && stats ? fmt(stats.spent) : "1.28M"}</div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "var(--accent-fg)", marginTop: 3 }}>{cur}{live && stats ? fmt(stats.avg) : "325"} avg order</div>
          </div>
          <div style={statCard}>
        ...
        {live && miners.length === 0 && <div style={{ fontSize: 13, color: "var(--text-muted)", textAlign: "center", padding: "16px 0" }}>No buyers yet.</div>}
