...
const ROWS: { key: PpBoolKey; sizeKey: PpSizeKey; label: string }[] = [
  { key: "shopName", sizeKey: "shopNameSize", label: "Shop name" },
  { key: "dateTime", sizeKey: "dateTimeSize", label: "Date & time" },
  { key: "buyerNum", sizeKey: "buyerNumSize", label: "Buyer #" },
  { key: "tiktokName", sizeKey: "tiktokNameSize", label: "TikTok name" },
  { key: "tiktokUser", sizeKey: "tiktokUserSize", label: "TikTok username" },
  { key: "comment", sizeKey: "commentSize", label: "Comment (center)" },
];
  ...
  return (
    <div>
      <div style={{ position: "sticky", top: 0, zIndex: 5, background: "var(--header-bg)", backdropFilter: "saturate(1.5) blur(14px)", color: "var(--on-header)", padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 18, letterSpacing: "-.01em" }}>LIVE print pattern</div>
      </div>
      ...
      <div style={{ padding: "12px 14px 16px" }}>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, boxShadow: "var(--shadow)", overflow: "hidden", marginTop: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 13px" }}>
            <span style={{ flex: 1, fontSize: 13.5, fontWeight: 700, color: "var(--text)" }}>SellerFlowLive logo</span>
            <span style={{ fontSize: 11.5, fontStyle: "italic", color: "var(--text-muted)" }}>Fixed default size</span>
          </div>
        </div>

        <button onClick={onBack} style={{ width: "100%", marginTop: 12, padding: "13px 0", border: "none", borderRadius: 12, background: "var(--accent)", color: "var(--accent-text)", fontFamily: "var(--font-ui)", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: "0 5px 14px var(--accent-soft)" }}>Save settings</button>
      </div>
    </div>
  );
}
