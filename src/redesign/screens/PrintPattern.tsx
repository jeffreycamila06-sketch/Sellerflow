...
const rowsFor = (t: RedesignT): { key: PpBoolKey; sizeKey: PpSizeKey; label: string; fixedSize?: boolean }[] => [
  ...
  { key: "dateTime", sizeKey: "dateTimeSize", label: t.rd_pp_date_time, fixedSize: true },
...
// Label shows the PRINTED level (printScaleLevel = the print path's own
// round+clamp), so a legacy stored 1.3 honestly reads "1×".
const sizeLabel = (n: number) => printScaleLevel(n) + "×";

// The only sizes the printer can actually produce (TSPL integer magnification;
// 3x is the largest that fits the label layouts). One tap = one whole step,
// starting from the PRINTED level of the current value — so a legacy 1.3
// steps 1.3→2 (up) or 1.3→1 (down), never 1.4.
export const SCALE_STEP_MIN = 1;
export const SCALE_STEP_MAX = 3;
export const stepScaleLevel = (current: number, dir: 1 | -1): number =>
        ...
        <div style={{ background: "var(--accent)", borderRadius: 16, padding: 11, boxShadow: "0 8px 22px var(--accent-soft)" }}>
          <div style={{ background: "#fff", borderRadius: 11, padding: "12px 14px", textAlign: "center" }}>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8 }}>
              ...
              {/* Date & time prints at a FIXED size (no scale in the print path) — preview mirrors that. */}
              {pp.dateTime && <span style={{ fontSize: 11, color: "#9795ad", whiteSpace: "nowrap" }}>Session: 05/22/2026 12:21PM</span>}
            </div>
            {/* Preview sizes = base px × printScaleLevel(value) — the SAME level the print path sends (preview = print). */}
            {pp.shopName && <div style={{ fontSize: 16 * printScaleLevel(pp.shopNameSize), fontWeight: 700, color: "#1c1a35", marginTop: 6 }}>Maria's Live Shop</div>}
            {pp.buyerNum && <div style={{ fontSize: 14 * printScaleLevel(pp.buyerNumSize), fontWeight: 700, color: "#1c1a35", marginTop: 3 }}>Buyer #12</div>}
            {pp.tiktokName && <div style={{ fontSize: 14 * printScaleLevel(pp.tiktokNameSize), fontWeight: 700, color: "#1c1a35", marginTop: 3 }}>Maria Santos</div>}
            {pp.tiktokUser && <div style={{ fontSize: 12 * printScaleLevel(pp.tiktokUserSize), fontWeight: 600, color: "#7c3aed", marginTop: 3 }}>@maria_live</div>}
            {pp.comment && <div style={{ fontFamily: "var(--font-mono)", fontSize: 12 * printScaleLevel(pp.commentSize), color: "#5a5872", marginTop: 7 }}>Comment</div>}
          </div>
        </div>
