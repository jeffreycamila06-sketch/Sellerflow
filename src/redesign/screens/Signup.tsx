...
export default function Signup({ onBack, onLegal, onRegister }: {
  ...
  const submit = async () => {
    ...
    if (r.ok && r.needsConfirm) { setOk("Account created — check your email to confirm, then log in."); setBusy(false); return; }
    if (!r.ok) { setErr(r.error || "Could not create your account."); setBusy(false); return; }
    ...
    <div style={{ minHeight: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ background: "var(--header-bg)", padding: "16px 16px 18px", color: "var(--on-header)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, opacity: 0.9, marginTop: 2 }}>Set up your shop — start capturing live orders.</div>
        </div>
      </div>
      ...
      <div style={{ padding: "16px 16px 22px" }}>
        ...
        <div style={groupCard}>
          <div><label style={label}>Shop name</label><input value={form.storeName} onChange={(e) => set("storeName", e.target.value)} placeholder="e.g. Maria's Live Shop" style={input} /></div>
          <div style={{ display: "flex", gap: 9 }}>
            <div style={{ flex: 1, minWidth: 0 }}><label style={label}>Owner name</label><input value={form.fullName} onChange={(e) => set("fullName", e.target.value)} placeholder="Full name" style={input} /></div>
            <div style={{ flex: 1, minWidth: 0 }}><label style={label}>Phone</label><input value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder="0917 000 0000" inputMode="tel" style={{ ...input, fontFamily: "var(--font-mono)", fontSize: 13 }} /></div>
          </div>
          <div><label style={label}>Email</label><input value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="you@email.com" autoComplete="email" inputMode="email" style={input} /></div>
        </div>

        <div style={{ ...groupLabel, margin: "16px 2px 9px" }}>PASSWORD</div>
        <div style={groupCard}>
          <div><label style={label}>Password</label><input type="password" value={form.password} onChange={(e) => set("password", e.target.value)} placeholder="Create a password (≥6 chars)" autoComplete="new-password" style={input} /></div>
          <div><label style={label}>Confirm password</label><input type="password" value={form.confirm} onChange={(e) => set("confirm", e.target.value)} placeholder="Re-enter password" autoComplete="new-password" style={input} /></div>
        </div>
