// Duplicate-order LAYER 3 (initial-comments feature) — restored (history) rows
// are DISPLAY-ONLY in the feed UI. Layers 1+2 are structural (initial comments
// never reach the Auto-Mode seam or feedRef — useLiveFeed.initialComments.test);
// this layer is an implementation promise, so it is pinned hard:
//   • a restored row renders ZERO <button> elements (no 1-Click, no Enterprise,
//     no printed/price UI) and is visually muted;
//   • the "— Earlier comments —" divider renders ONCE, above the first restored
//     row (history block sits below the live block);
//   • the 🛒 basket badge (session-derived) still shows on history rows;
//   • live rows keep their full action row — manual capture flow intact.
...
describe("Dashboard restored rows (duplicate-order layer 3)", () => {
  ...
  it("the Earlier-comments divider renders ONCE, above the FIRST restored row only", () => {
    ...
    const matches = (container.textContent!.match(/Earlier comments/g) || []).length;
  ...
  it("🛒 basket badge still renders on a restored row (session-derived, meaningful on history)", () => {
